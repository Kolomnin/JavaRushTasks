...
Requirements:
1. Метод runWorkingProcess() должен вызывать pilot(Human human) 2 раза. В качестве аргументов передать
все объекты типа Human из списка astronauts.
2. Метод runWorkingProcess() должен вызывать createDirection(Dog dog) 1 раз. В качестве аргумента передать
объект типа Dog из списка astronauts.
3. Метод runWorkingProcess() должен вызывать research(Cat cat) 1 раз. В качестве аргумента передать объект
типа Cat из списка astronauts.
*/
...
public class Solution {
    ...
    public static void runWorkingProcess() {
        Human human1 = (Human) astronauts.get(0); //экземплят списка (Human) записывается в скобках
        pilot(human1);
